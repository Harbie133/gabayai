...
function renderFollowupQuestions(questions) {
  ...
  questions.forEach((q, idx) => {
    ...
    // Handle object options if necessary
...
function renderResults() {
  ...
  // Render Red Flags (existing logic)
  ...
  // Default 5 visible, "Show more" for the rest
  ...
  if (allConds.length > DEFAULT_VISIBLE) {
    ...
    more.addEventListener("click", () => {
      ...
      renderResults(); // Re-render list
    });
    listDiv.appendChild(more);
  }

  // Show details for the first one automatically
...
// Helper function to collect data with language (iniwan ko, hindi ginagalaw yung global mo)
